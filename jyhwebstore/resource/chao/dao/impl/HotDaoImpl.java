		...
		try {
			if(!conn.isClosed()){
				...
				while(rs.next()){
					...
					hot.setHotdatestart(rs.getDate("hotdatestart"));
					hot.setHotdatend(rs.getDate("hotdatend"));
	...
	public boolean insertHot(Hot hot, Connection conn) {
		...
		try {
			if(!conn.isClosed()){
				...
				ps.setDate(3, new java.sql.Date(hot.getHotdatestart().getTime()));
			    ps.setDate(4, new java.sql.Date(hot.getHotdatend().getTime()));
	...
	public boolean modifyHot(Hot hot, Connection conn) {
		...
		try {
			if(!conn.isClosed()){
			 ...
			 ps.setDate(2, new java.sql.Date(hot.getHotdatestart().getTime()));
			 ps.setDate(3, new java.sql.Date(hot.getHotdatend().getTime()));
